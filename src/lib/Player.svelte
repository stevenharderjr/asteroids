<script>
  ...
  import { force, accelerate } from '../utils/vector.js'
  ...
  let bounceEffect = .8;
  let rotationSpeed = 0;
  let rotationAccel = 1.2;
  let maxRotationSpeed = 6;
  let rotation = 0;
  ...
  function respondToInput() {
    if (keys.ArrowUp) {
      ...
      heading = accelerate(heading, { x: Math.cos(rads), y: Math.sin(rads)})
    }
    // if (keys.ArrowDown) {
    //   if (yStep < 0) {
    //     yStep *= (1 - accel);
    ...
    if(keys.Space) dispatch('shoot', force(heading, 20));
  }
